 ...
 * @author Retha@wii
 */
public class PrintFakultas extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            List listFakultas = new ArrayList();
            Iterator iter = new FakultasDAO().getAllFakultasDanJumlahMahasiswa();
            long total = 0;

            for (Iterator it = iter; it.hasNext();) {
                Object[] row = (Object[]) it.next();
                Fakultas fak = new Fakultas();
                fak.setKode((String)row[0]);
                fak.setNama((String)row[1]);
                fak.setJumlah((Long)row[2]);
                total = total + (Long)row[2];
                listFakultas.add(fak);
            }
            ...
            JRBeanCollectionDataSource dataSource = new JRBeanCollectionDataSource(listFakultas);
            System.out.println("1 " + getServletContext().getRealPath("report/admin/fakultas.jrxml"));
            // Compile JRXML menjadi Jasper
            JasperReport jasperReport = JasperCompileManager.compileReport(getServletContext().getRealPath("report/admin/fakultas.jrxml"));
