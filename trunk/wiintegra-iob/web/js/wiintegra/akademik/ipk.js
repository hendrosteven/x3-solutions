...
Ext.onReady(function(){
  ...
  // ------------------------ tabel jadwal -------------------------------------
  IPKsDataStore = new Ext.data.Store({
      id: 'IPKsDataStore',
  ...
  IPKsColumnModel = new Ext.grid.ColumnModel(
    [
    ...
    IPKsColumnModel.defaultSortable= true;

   IPKListingEditorGrid =  new Ext.grid.EditorGridPanel({
      loadMask: {msg: 'Loading...'},
      width:655,
      height:445,
      //title:'Data Matakuliah',
      frame:true,
      id: 'IPKListingEditorGrid',
      store: IPKsDataStore,
      viewConfig: {
          forceFit:true
      },
      ...
      tbar: [
          {
            text: 'Print',
            tooltip: 'Print Data IPK',
            handler: print,
            iconCls:'print'
          }, '-', {
            text: 'Cetak ke Excel',
            tooltip: 'Cetak Tranksrip ke file Excel',
  ...
  DataTotalForm = new Ext.FormPanel({
  ...
  var fieldsetNilai = new Ext.form.FieldSet({
      ...
      layout: 'form',
      items:[
              TotalAngkaField, TotalSKSField, IPKField
      ]
  });
   //---------------------------------------------------------------------------
 ...
 //-----------------------------------------------------------------------------
  ...
  TagihanForm = new Ext.FormPanel({
        ...
        labelAlign: 'top',
        bodyStyle:'padding:5px, background-color: #eef4f8',
        width: 300,
        border: false,
        height: 445,
        autoScroll: true,
  ...
  panel = new Ext.Panel({
        ...
        hideBorders: true,
        bodyStyle:'background-color: #eef4f8',
        title:'Index Prestasi Kumulatif / Tranksrip',
        items: [{
            layout:'column',
            //width: 606,
            items:[{
    ...
    function tampilDataTagihan() {
       ...
       Ext.Ajax.request({
          ...
          success: function(result, request){
              ...
              semester = jsonData.currentSemester;
          },
  ...
  var teks = 'Untuk mencetak Transkrip Mahasiswa dalam format PDF, klik tombol "Print".<br/>' +
             'Untuk mencetak Transkrip Mahasiswa ke dalam Microsoft Excel, klik tombol "Cetak Excel".<br/>' +
             'Pastikan Anda telah menentukan Mahasiswa yang dimaksud terlebih dahulu dengan memilih drop down Nomor Induk dan mengklik tombol "Tampilkan".';
  HelpWindow = new Ext.Window({
    ...
    function print(){
        if(NomorIndukField.getValue() == '')
            ...
            window.open('printTranskrip?idMhs=' + NomorIndukField.getValue(), '_blank', '');
        //win.show();
    }
    ...
    function printExcel(){
        if(NomorIndukField.getValue() == '')
            ...
            window.open('printTranskripExcel?idMhs=' + NomorIndukField.getValue(), '_blank', '');
        //win.show();
    }
    ...
    function print2() {
        ...
        Ext.ux.GridPrinter.print(IPKListingEditorGrid, data);
    }
});
    ...
    var html = new Ext.XTemplate(
      ...
      '<html>',
        '<head>',
          ...
          '<title>Index Prestasi Kumulatif</title>',
        '</head>',
        '<body>',
        'Bagian Admisi dan Registrasi',
        '<center><h1>Index Prestasi Kumulatif<br>',
