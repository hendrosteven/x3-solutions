...
public class KegiatanPemerintahDAOImpl implements KegiatanPemerintahDAO {
    ...
    public void insert(KegiatanPemerintah kp) throws Exception {
        String sql = "INSERT INTO kegiatan_pemerintah(nama,jalan,kota,tahun,kelurahan_id,kecamatan_id,"
                + "sumber_dana_id,bidang_usaha_pemerintah_id,nilai_investasi) "
                + "VALUES(?,?,?,?,?,?,?,?,?)";
    ...
    public void update(KegiatanPemerintah kp) throws Exception {
        String sql = "UPDATE kegiatan_pemerintah SET nama=?,jalan=?,kota=?,tahun=?,kelurahan_id=?,"
                + "kecamatan_id=?,sumber_dana_id=?,bidang_usaha_pemerintah_id=?,nilai_investasi=? WHERE id=?";
        PreparedStatement ps = null;
        try {
            ps = this.conn.prepareStatement(sql);
            ps.setString(1, kp.getNama());
    ...
    public KegiatanPemerintah get(int id) throws Exception {
        ...
        String sql = "SELECT id,nama,jalan,kota,tahun,kelurahan_id,kecamatan_id,sumber_dana_id,bidang_usaha_pemerintah_id,nilai_investasi FROM "
                + "kegiatan_pemerintah WHERE id=?";
        PreparedStatement ps = null;
        try {
            ps = this.conn.prepareStatement(sql);
            ps.setInt(1, id);
