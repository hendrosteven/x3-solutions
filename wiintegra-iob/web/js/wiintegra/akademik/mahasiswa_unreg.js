...
Ext.onReady(function(){
  ...
  MahasiswasDataStore = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
            ...
            baseParams:{task: "LISTINGUNREG_LIMIT"}, // this parameter is passed for any HTTP request
      reader: new Ext.data.JsonReader({
        root: 'daftarMahasiswa',
        totalProperty: 'total'
      },[
        {name: 'id', type: 'int', mapping: 'id'},
        {name: 'idMhs', type: 'int', mapping: 'idMhs'},
  ...
  MahasiswasColumnModel = new Ext.grid.ColumnModel(
   ...
   MahasiswaListingEditorGrid =  new Ext.grid.EditorGridPanel({
      ...
      width:900,
      height:480,
      title:'Biodata Mahasiswa yang sudah Terregistrasi',
      frame:true,
      ...
      tbar: [
          ...
          , '-', { // Added in Tutorial 6
            text: 'Unregistrasi',
            tooltip: 'Batalkan Registrasi Mahasiswa ke Semester sekarang',
            iconCls:'registrasi',                      // reference to our css
            handler: confirmUnregistrasi
          }, '-', { // Added in Tutorial 6
            text: 'Print',
            tooltip: 'Print Data Mahasiswa yang sudah Terregistrasi ke Semester sekarang',
            handler: print,
            iconCls:'print'
          }, '-', {
            text: 'Help',
...
//------------ ambil data current semester dulu baru dirender ke panel ---------
       Ext.Ajax.request({
          ...
          success: function(response){
             ...
             semester = result;
             MahasiswaListingEditorGrid.setTitle('Data Mahasiswa yang telah Terregistrasi pada '+semester);
...
//------------------------------------------------------------------------------
  ...
  //MahasiswaListingWindow.show();
//  MahasiswaListingEditorGrid.render("grid");
//  MahasiswasDataStore.load();
  //MahasiswaListingEditorGrid.on('afteredit', saveTheMahasiswa);

  var teks = 'Untuk membatalkan registrasi mahasiswa pada semester terbaru, klik Mahasiswa-mahasiswa yang akan dibatalkan dan klik tombol Unregistrasi.' +
  ...
  function confirmUnregistrasi(){
    /*if(PembayaranListingEditorGrid.selModel.getCount() == 1) // only one Pembayaran is selected here
    {
      ...
      Ext.MessageBox.confirm('Konfirmasi','Anda akan membatalkan '+MahasiswaListingEditorGrid.selModel.getCount()+' mahasiswa untuk diregistrasikan semester ini. Lanjutkan?', unregistrasi);
    } else {
      Ext.MessageBox.alert('Peringatan','Anda harus memilih data Mahasiswa terlebih dahulu!');
    }
  }

  function unregistrasi(btn){
    if(btn=='yes'){
         ...
         Ext.Ajax.request({
            ...
            success: function(response){
              ...
              case 1:  // Success : simply reload
                ...
                break;
              default:
                Ext.MessageBox.alert('Peringatan','Tidak dapat membatalkan registrasi Mahasiswa!');
                break;
              }
            },
            failure: function(response){
    ...
    function print(){
        window.open('printMahasiswaUnreg?idFakultas=' + FakultasSearchField.getValue(), '_blank', '');
    }
});